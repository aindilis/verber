package Verber::Federated::System::PaperlessOffice1;

# FIXME: Add stuff to interface with paperless office temporal
# planning domains, such as which papers have to be acted on by a
# certain date, which documents have to be reviewed still, which piles
# need to be scanned, etc.
...
has 'RegisteredWorldNames' => (is => 'rw',isa => 'ArrayRef',default => sub {[qw(SYSTEM_PAPERLESSOFFICE1)]});
...
override 'GenerateDomainPrototype' => sub {
  ...
  # FIXME: refactor to different initialization section
  if ($args{Context}) {
...
sub Generate {
  ...
  if (defined $message) {
	...
	if ($item eq "end-opportunity") {
	  $sdt = $self->Problem->StartDate;
	} elsif ($item eq "start-opportunity") {
	  $edt = $self->Problem->StartDate + DateTime::Duration->new(years => 10);
    ...
    $self->Problem->Problem("SYSTEM_PAPERLESSOFFICE1");
    $self->Problem->Domain("SYSTEM_PAPERLESSOFFICE1");
    ...
    # print out the resulting domain
