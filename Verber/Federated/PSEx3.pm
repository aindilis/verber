package Verber::Federated::PSEx3;
...
has 'RegisteredWorldNames' => (is => 'rw',isa => 'ArrayRef',default => sub {[qw(psex3)]});
...
has 'Context' =>
  (
   is  => 'rw',
   isa => 'Str',
   default => sub { "Org::FRDCSA::Verber::PSEx2::Do" },
  );
...
sub Generate {
  ...
  if (defined $message) {
    ...
    foreach my $assertion (@$assertions) {
      my $pred = $assertion->[0];
      if ($pred eq "depends") {
	print Dumper({WTFHomie => $assertion->[1]});
    ...
    $self->Problem->Problem("PSEX3");
    $self->Problem->Domain("PSEX3");
