...
(defun all-sits (sit)
  (if (null (time:after sit))
      (list sit)
    (append (list sit)
	    (let ((nsits (time:after sit)))
	      (loop for nsit in (listify nsits)
		appending (all-sits nsit))))))

(defun all-sit-props (sit &optional rel)
  (let* ((sits (all-sits sit))
	 (props (loop for sit in sits
		   appending (situation-propositions (eval sit)))))
...
(define (durative-action jdam-travel-to-tgt)
    ...
    :effect (and (at end (is-located ?o unknown-location))
		 (at end (tgt-located ?o ?obj)))
    :probability 0.75
...
#|
;;;From dock or harbor, with a sequence
;;; Use this one when detailing the action
...
(define (durative-action uav-launch-and-fly-to)
    ...
    :duration time-to-travel-uv
    :effect (and (at end (has-launch-state ?o in-the-air))
...
;;; 8/19 2015
;;; When doing individual take off go and land, the land always uses this action.
;;; I tried setting a lower prob but it still chooses this instead of uav-travel-to.
;;; So in a demo you can't have both.  In the meantime I made another one below.
#|(define (durative-action uav-fly-to-and-land)
    :parameters (?o - unmanned-aerial-vehicle
...
(define (durative-action uav-fly-to-and-land2)
    :parameters (?o - unmanned-aerial-vehicle
		 ?end-loc - base-location)
    :condition (and (at start (not (instance ?o smart-munition)))
                    (at start (has-launch-state ?o in-the-air)))
    :duration time-to-travel-uv
    :probability 0.75
    :effect (and (at end (has-launch-state ?o not-launched))
                 (at end (is-located ?o ?end-loc)))	
    :comment "?o travels from ?start-loc to ?end-loc")|#

(define (durative-action usv-travel-to)
    :parameters (?o - unmanned-surface-vehicle
		 ?end-loc - (or water-area base-location))
    :vars (?start-loc - (is-located ?o))
    :effect (at end (is-located ?o ?end-loc))
    :duration time-to-travel-uv
    :comment "?o travels from ?start-loc to ?end-loc")
...
(define (durative-action surveil)
    :parameters (?o - unmanned-surface-vehicle ?t - (and physical-entity (not sensor)(not weapon)(not water-area)))    
    :vars (?base-loc - base-location)
    :condition (at start (is-located ?o ?base-loc))
    :expansion (sequential
		(tgt-located ?o ?t)
