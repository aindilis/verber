...
(in-package :ap)
(define (domain dod-domain)
  (:requirements :multi-agent :durative-actions)
  (:extends dodi)
  (:predicates 
   (Delay10_a ?o - agent ?s - state)
   (tasks-done1 ?lcs - (or agent ship))(tasks-done2 ?lcs - (or agent ship))(tasks-done3 ?lcs - (or agent ship))(tasks-done4 ?lcs - (agent ship))
   (did-surveil ?lcs - agent)(took-off&landed ?o - agent)
