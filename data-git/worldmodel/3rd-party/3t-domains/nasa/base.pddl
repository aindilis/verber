...
(define (domain base)
  ...
  (:types
     ...
     substance environment location consumable pump computer power-channel power-source control-bus robot weapon sensor actuator equipment transport light antenna vehicle motor celestial-body network social-entity enclosure furniture - physical-entity
     ...
     boolean-value size color age rate
     state role geometry configuration activity-type
     capability - abstract-entity
     mode - state
   )  
  (:constants
    ...
    black white green blue red yellow violet indigo orange  - color
    cubic hexagonal  - geometry
    nothing  - physical-entity
    yes X untripped unsynched unknown tripped transitioning static standby standby single_bed operational 
    on okay offline off not_armed no-change no no night min-ops mdm2 mdm1 leak-rcvy-fdir-state-inhibited 
    leak-rcvy-fdir-state-enabled invalid in-sync inprogress init inhibited inhibit incrementing inactive 
    in_progress fire fail-rcvy-fdir-state-inhibited fail-rcvy-fdir-state-enabled fail 
    enabled enable2 enable1 enable dual_bed deactivate-input-undervolt-trip-recovery 
    deactivate-input-undervolt-trip deactivate-input-undervolt-command-rejection deactivate-commanded-bit 
    deactivate day complete 
    ;;;closed closed 
    close blank-on-off-status blank-command-status blank armed arm activate ?  - enumeration
  ...
  (:predicates
    ...
    ;;(has-color ?pe - physical-entity ?c - color)
