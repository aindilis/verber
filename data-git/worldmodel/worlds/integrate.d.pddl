...
(define
 ...
 (:durative-action use-tool :parameters
  (?p - person ?t - tool ?l - location) :duration
  (= ?duration 1) :condition
  (and
   (over all
 ...
 (:durative-action work-fifteen-minutes :parameters
  (?p - person ?c - computer ?o - office ?d - date) :duration
  (= ?duration 0.25) :condition
  (and
   (over all
    (ready-for-work ?p))
   ...
   (over all
    (<=
     (food-ingested Andrew-Dougherty) 0))) :effect
  (and
